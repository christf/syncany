...
/**
 * This is a bittorrent client based on mpetazzonis simple example that is able to manage multiple bt-clients
...
public class QueueingClient extends Observable implements Runnable, AnnounceResponseListener, IncomingConnectionListener, PeerActivityListener {
	 ...
	 */
	public QueueingClient(InetAddress address, SharedTorrent torrent, int seedport) throws UnknownHostException, IOException {
		...
		logger.info("BitTorrent client [{}] for {} started and " + "listening at {}:{}...", new Object[] { this.self.getShortHexPeerId(),
				this.torrent.getName(), this.self.getIp(), this.self.getPort() });
	...
	/**
	 * Set the maximum download rate (in kB/second) for this torrent. A setting
	 * of <= 0.0 disables rate limiting.
	 * 
	 * @param rate
	 *            The maximum download rate
	 */
	...
	/**
	 * Set the maximum upload rate (in kB/second) for this torrent. A setting of
	 ...
	 * 
	 * @param seed
	 *            Seed time in seconds after the download is complete. Pass
	 ...
	 * 
	 * @param wait
	 *            Whether to wait for the client execution thread to complete or
	 *            not. This allows for the client's state to be settled down in
	 *            one of the <tt>DONE</tt> or <tt>ERROR</tt> states when this
	 *            method returns.
	 */
	...
	public void run() {
		...
		while (!this.stop) {
			optimisticIterations = (optimisticIterations == 0 ? QueueingClient.OPTIMISTIC_UNCHOKE_ITERATIONS : optimisticIterations - 1);

			rateComputationIterations = (rateComputationIterations == 0 ? QueueingClient.RATE_COMPUTATION_ITERATIONS : rateComputationIterations - 1);
			...
			try {
				Thread.sleep(QueueingClient.UNCHOKING_FREQUENCY * 1000);
			}
			catch (InterruptedException ie) {
	 ...
	 * includes the number of choked peers, number of connected peers, number of
	 * known peers, information about the torrent availability and completion
	 * and current transmission rates.
	 * </p>
	 */
	public synchronized void info() {
		...
		logger.info(
				...
				new Object[] { this.getState().name(), this.torrent.getCompletedPieces().cardinality(), this.torrent.getPieceCount(),
						String.format("%.2f", this.torrent.getCompletion()), this.torrent.getAvailablePieces().cardinality(),
	 ...
	 * 
	 * <p>
	 * Returns a peer comparator based on either the download rate or the upload
	 * rate of each peer depending on our state. While sharing, we rely on the
	 * download rate we get from each peer. When our download is complete and
	 * we're only seeding, we use the upload rate instead.
	 * </p>
	 * 
	 ...
	 * them to get more data more quickly. If one becomes interested, it takes a
	 * downloader's place as one of the four top downloaders (i.e. we choke the
	 * downloader with the worst upload rate).
	 * </p>
	 * 
	 * @param optimistic
	 *            Whether to perform an optimistic unchoke as well.
	 */
	private synchronized void unchokePeers(boolean optimistic) {
		...
		for (SharingPeer peer : bound.descendingSet()) {
			if (downloaders < QueueingClient.MAX_DOWNLOADERS_UNCHOKE) {
	 ...
	 * 
	 * @param channel
	 *            The connected socket channel to the remote peer. Note that if
	 *            the peer somehow rejected our handshake reply, this socket
	 *            might very soon get closed, but this is handled down the road.
	 * @param peerId
	 *            The byte-encoded peerId extracted from the peer's handshake,
	 *            after validation.
	...
	public void handleNewPeerConnection(SocketChannel channel, byte[] peerId) {
		...
		try {
			...
			logger.debug("New peer connection with {} [{}/{}].", new Object[] { peer, this.connected.size(), this.peers.size() });
		}
		catch (Exception e) {
	 ...
	 * 
	 * @param peer
	 *            The peer we were trying to connect with.
	 * @param cause
	 *            The exception encountered when connecting with the peer.
	 */
	@Override
	...
	@Override
	public void handleBitfieldAvailability(SharingPeer peer, BitSet availablePieces) { /* Do nothing */
	}

	@Override
	public void handlePieceSent(SharingPeer peer, Piece piece) { /* Do nothing */
	}
	 ...
	 * 
	 * @param peer
	 *            The peer we got the piece from.
	 * @param piece
	...
	public void handlePieceCompleted(SharingPeer peer, Piece piece) throws IOException {
		synchronized (this.torrent) {
			if (piece.isValid()) {
				...
				logger.debug("Completed download of {} from {}. " + "We now have {}/{} pieces", new Object[] { piece, peer,
						this.torrent.getCompletedPieces().cardinality(), this.torrent.getPieceCount() });
	...
	public void handlePeerDisconnected(SharingPeer peer) {
		if (this.connected.remove(peer.hasPeerId() ? peer.getHexPeerId() : peer.getHostIdentifier()) != null) {
			logger.debug("Peer {} disconnected, [{}/{}].", new Object[] { peer, this.connected.size(), this.peers.size() });
		}
	...
	public void handleIOException(SharingPeer peer, IOException ioe) {
		logger.warn("I/O error while exchanging data with {}, " + "closing connection with it!", peer, ioe.getMessage());
		peer.unbind(true);
	}
	 ...
	 * 
	 * <p>
	 * When the download is complete, the client switches to seeding mode for as
	 * long as requested in the <code>share()</code> call, if seeding was
	 * requested. If not, the {@link ClientShutdown} will execute immediately to
	...
	public static class ClientShutdown extends TimerTask {
		...
		private final QueueingClient client;
		private final Timer timer;

		public ClientShutdown(QueueingClient client, Timer timer) {
			this.client = client;
			this.timer = timer;
		}
